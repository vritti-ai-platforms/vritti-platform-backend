...
The **Region module** in the vritti-cloud cloud-server is the original complete working example:
- `src/modules/admin-api/region/controllers/region.controller.ts`
- `src/modules/admin-api/region/services/region.service.ts`
- `src/modules/admin-api/region/repositories/region.repository.ts`
- `src/modules/admin-api/region/dto/response/regions-response.dto.ts`

> Note: The original reference lives at `../vritti-cloud/apps/cloud-server/` — paths above are relative to this project's `src/` directory for local modules following the same pattern.
